...
class ReportsController extends Controller
{
    ...
    public function view_list_report_daily_all(){
        ...
        $rows=\DB::select(\DB::raw($query));
        //return $rows;
        return view('reports.index_list_attentions_full_users')->with('list_daily',$rows);
    }
    public function view_list_report_range_daily_all(){
        return view('reports.insert_date_range_full');
    }
    public function send_range_dates_controller_full_user(Request $request){
        $query = "SELECT p.ci_paciente, p.nombres, p.ap_paterno, p.ap_materno,ht.start_time, sapp.name_state_appointments, mapp.appointment_description FROM medical_appointments mapp
                        INNER JOIN medical_assignments mass
                            ON mass.id_medical_assignments = mapp.id_medical_assignments
                        INNER JOIN users us
        ...
        //$rows=\DB::select(\DB::raw($query));            
        $rows=\DB::select(\DB::raw($query),array('date_start'=>$request->date_start,'date_end'=>$request->date_end));
        //return $rows;
        //return (Auth::user()->id);
        $var1 = [$request->date_start, $request->date_end];
        //return $var1;
        return view('reports.index_list_range_dates_full_users')->with('list_daily',$rows)->with('dates',$var1);
    }
}